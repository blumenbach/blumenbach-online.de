...
class BlumenbachOnline_readonly extends tslib_pibase {
  ...
  function __construct() {
    $this->BlumenbachOnline_readonly();
  }
...
/*    if ($show_deletionlink) {
    ...
    foreach ($mediafiles as $nr => $data) {
      ...
      $content .= "\n" . '<div id="bild_' . $id . '"><img onClick="window.open(\'/' . $this->workpath_rel . $uri . '\')" src="/' . $this->workpath_rel . $uri . '" title="' . $uri . '" alt="' . $uri . '" style="width:80px;border:0px;float:left;border:1px solid gray;padding:1px;">&nbsp;&nbsp;';
          ...
          */
          if ($special_inputfields[$bez] != "") {
            ...
            //$content .= '<tr><td' . $bgcolor . '>' . $label . ':</td><td><textarea name="' . $bez . '[]" style="width:400px;height:' . $height . 'px;">' . $rowobj[$bez] . '</textarea></td></tr>' . "\n";
              ...
              $content .= '<tr><td' . $bgcolor . '>' . $label . ':</td><td>'.$rowobj[$bez].'</td></tr>'. "\n";
    ...
    */
    ...
    if ($block_bez == "tx_bolonline_PartIII") { //=gelb
      ...
      $content .= "<tr><td><div style='color:blue'>Zuordnung zu Blau:</div><div style='color:black;display:none;
        width:300px;padding:2px' id='" . $selection_name . "_message'></div></td><td colspan='2'><span id='" .
      ...
      foreach ($ids_blau as $nr => $a_data) {
        ...
        $content .= "<span" . $selected . " value='" . $spid . "'>" . $bezeichner . "</span>";
      }
      $content .= "</span>";
...
/*      $content .= "<span style='font-size:0.9em'>(Auswahl mit gedr&uuml;ckter Strg-Taste)</span><br />
      ...
      $content .= "<tr><td><div style='color:red'>Zuordnung zu Rot:</div><div style='color:black;display:none;width:300px;
        padding:2px' id='" . $selection_name . "_message'></div></td><td colspan='2'><span id='" . $selection_name
...
/*      $content .= "<span style='font-size:0.9em'>(Auswahl mit gedr&uuml;ckter Strg-Taste)</span><br />
    ...
    if ($block_bez == "tx_bolonline_PartIV") { //=orange
      ...
      $selection_name . "_message'></div></td><td colspan='2'><span id='" .
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/bolonline/pi2/class.BlumenbachOnline_readonly.php']) {
  include_once ($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/bolonline/pi2/class.BlumenbachOnline_readonly.php']);
}
?>
