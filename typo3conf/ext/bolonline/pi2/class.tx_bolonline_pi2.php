...
require_once ('class.BlumenbachOnline_readonly.php');

class tx_bolonline_pi2 extends tslib_pibase {

    var $prefixId = 'tx_bolonline_pi2'; // Same as class name
    var $scriptRelPath = 'pi2/class.tx_bolonline_pi2.php'; // Path to this script relative to the extension dir.
    ...
    function main($content, $conf) {
        $this->Bol = new BlumenbachOnline_readonly();
        $this->conf = $conf;
...
//		include_once("./bolonline.main.conf.php");
        ...
        if($_SESSION['hauptkategorie_id'] == "all") {
          $_SESSION['hauptkategorie_id'] = "";
        }
        ...
        $xajax->setLogFile("/var/www/blumenbach-online.de/typo3conf/ext/bolonline/test_ajax.log");
...
*/
        $xajax->registerExternalFunction('setFields', t3lib_extMgm :: siteRelPath('bolonline') . '/pi2/fileupload.ajax.incl.php');
...
/*        $xajax->registerExternalFunction('getDBDumpfile', t3lib_extMgm :: siteRelPath('bolonline') . '/pi1/datensicherung.php');
        ...
        $xajax->registerExternalFunction('getCERLData', t3lib_extMgm :: siteRelPath('bolonline') . '/pi1/cerlSearch.ajax.incl.php');
        $xajax->registerExternalFunction('putCERLValues', t3lib_extMgm :: siteRelPath('bolonline') . '/pi1/cerlSearch.ajax.incl.php');
*/
...
//        if(time()%2==0) die();
        ...
        //include_once 'datensatz.copier.incl.php';
        
        /*$content .= '<tr><td colspan="2">
            <form action="' . $this->pi_getPageLink($GLOBALS['TSFE']->id) . '" method="post" style="padding: 5px;">
                Datensatz kopieren:
                <input type="text" name="kdatid" placeholder="Kerndaten-ID" />
...
//        if ($_SESSION['todo'] == 'update') {
        // the object filter chooser
          $content .= '<tr><td colspan="2"><h3>Datensatz Betrachten</h3></td></tr>';
          $content .= '<script type="text/javascript" src="' . t3lib_extMgm :: siteRelPath('bolonline') . '/pi1/bolobjectFilter.js"></script>
                    <tr>
                      <td colspan="2" style="text-align:right;">
                        Filter: <input type="text" onkeyup="bolObjectFilter(this.value);"/> <span id="objectFilterCount"></span> Objekte in Auswahlliste
                     </td>
                    </tr>'; 
            $content .= '<tr><td><form name="update" action="' . $this->pi_getPageLink($GLOBALS['TSFE']->id) . '" method="POST">';
            $content .= '<input type="hidden" name="todo" value="update">';
            $content .= '<select id="bolObjectList" style="background-color:#fda92a;width:100%;padding:2px" name="kerndaten_id[]" onChange="document.update.submit()">
																		<option value="">bitte wählen</option>';
...
//			$content .= $this->Bol->dump($datensatz_ids);
            foreach ($datensatz_ids as $id => $value) {
                ...
                $content .= '<option value="' . $id . '"' . ($_SESSION['update_id'] == $id ? " selected" : "") . '>' . substr($value, 0, 250).'</option>';
            }
            $content .= '</select>';
            $content .= '</form></td></tr>';
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/bolonline/pi2/class.tx_bolonline_pi2.php']) {
    include_once ($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/bolonline/pi2/class.tx_bolonline_pi2.php']);
}
